...
#'
#'@param timeFormat
#'  Single character string describing how the time is formatted 
#'  on the computer (and by extension how it is exported in the 
#'  PAR file by MACRO shell). An attempt is made when the package 
#'  is loaded to set this option, using the system settings 
#'  (as seen from \code{echo \%DATE\%} and \code{echo \%TIME\%}).
#'  Default (prior to package loading) is \code{"\%Y-\%m-\%d"} for 
#'  dates like \code{"2014-06-03"} (4-digit year, 2 digit month 
#'  and 2 digit day). Other examples are \code{"\%Y/\%m/\%d"} (slash 
#'  as separator) or \code{"\%d-\%m-\%Y"} (day-month-year format).
#'  It is also possible to include time, using \code{"\%H:\%M"}, 
#'  (hours:minutes), for example \code{"\%Y-\%m-\%d \%H:\%M"}. Notice 
#'  that MACRO may or may not export the hour:minute elements 
#'  of the time, if simulation starts or ends at 00:00.
#'
#'@param tz
...
rmacroliteSimPeriod.macroParFile <- function( 
    ...
    if( grepl( x = .metPeriod, pattern = "- " ) ){
        ...
        .metPeriod <- c( 
            substr( x = .metPeriod, start = 1, stop = 16 ), 
            substr( x = .metPeriod, start = nchar( .metPeriod ) - 15, stop = nchar( .metPeriod ) )
        )   
    }   
    ...
    #   Format output
    out <- list(
        "sim"           = .rml_cPOSIXct( .start, .end ), 
        "metPeriod"     = .metPeriod  
        # "rainBinPeriod" = rainBinPeriod, 
        # "metBinPeriod"  = metBinPeriod 
    )   
...
# ## #
# .rml_testDateFormat <- function( error = FALSE, verbose ){ 
    ...
    # .rml_logMessage( 
        # "rmacro: system date-time format identified as %s", 
        # verbose = verbose, 
...
`rmacroliteApplications<-.macroParFile` <- function( 
    ...
    if( !("list" %in% class( value )) ){
        stop( sprintf( 
            "Argument 'value' should be a list with %s items (now class: %s)", 
            length( value_expect ), 
            paste( class( value ), collapse = ", " ) ) )
    ...
    if( !(length( keep0conc ) == 1L) ){
        stop( sprintf(
            ...
            length( keep0conc ) # WORK ON PROGRESS 
        ) ) 
    }   
